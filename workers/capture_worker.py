...
from capture.window import get_active_window_title, get_active_app_name
...
class CaptureWorker:
        ...
        while self._running:
            ...
            try:
                if hasattr(self, '_audio_worker') and self._audio_worker:
                    from capture.window import get_active_app_name as _get_app
                    _app = _get_app()
                    if _app:
    ...
    async def _capture_tick(self, trigger="periodic"):
        ...
        try:
            ...
            try:
                from integrations.smart_notify import check as notify_check
